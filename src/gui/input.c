...
void	save_sphere(t_view *s, int num)
{
	...
	num == 6 ? s->space->cl_figures[s->rr.fl.y].texture = par_input() : 0;
	redraw(s);
}
...
void	save_plane(t_view *s, int num)
{
	if (s->flag != 0 || s->rr.fl.y == -1)
		return ;
	num == 0 ? s->space->cl_figures[s->rr.fl.y].point.x = par_input() : 0;
	num == 1 ? s->space->cl_figures[s->rr.fl.y].point.y = par_input() : 0;
	num == 2 ? s->space->cl_figures[s->rr.fl.y].point.z = par_input() : 0;
	...
	num == 8 ? s->space->cl_figures[s->rr.fl.y].texture = par_input() : 0;
	redraw(s);
}
