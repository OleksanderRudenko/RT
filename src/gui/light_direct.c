...
void	light_dir_properties(t_view *s)
{
	...
	s->dir.dir_rect[0] = make_rect(10, 150, 100, 32);
	s->dir.dir_tex[0] = create_text(s, "X-Dir", 2, 32);
	s->dir.dir_pr_rect[0] = make_rect(120, 150, 60, 32);
	s->dir.dir_prop[0] =  tf(s, 0, 2, 32);
	s->dir.dir_rect[1] = make_rect(10, 200, 100, 32);
	s->dir.dir_tex[1] = create_text(s, "Y-Dir", 2, 32);
	s->dir.dir_pr_rect[1] = make_rect(120, 200, 60, 32);
	s->dir.dir_prop[1] =  tf(s, 0, 2, 32);
	s->dir.dir_rect[2] = make_rect(10, 250, 100, 32);
	s->dir.dir_tex[2] = create_text(s, "Z-Dir", 2, 32);
	s->dir.dir_pr_rect[2] = make_rect(120, 250, 60, 32);
	s->dir.dir_prop[2] =  tf(s, 0, 2, 32);
	s->dir.dir_rect[3] = make_rect(10, 300, 100, 32);
	s->dir.dir_tex[3] = create_text(s, "Inten", 2, 32);
	s->dir.dir_pr_rect[3] = make_rect(120, 300, 60, 32);
	s->dir.dir_prop[3] =  tf(s, 0, 2, 32);
}

void	init_dir_light_prop(t_view *s)
{
	s->dir.dir_prop[0] = tf(s, s->space->cl_lights[s->rr.fl.b].direction.x, 2, 32);
	s->dir.dir_prop[1] = tf(s, s->space->cl_lights[s->rr.fl.b].direction.y, 2, 32);
	s->dir.dir_prop[2] = tf(s, s->space->cl_lights[s->rr.fl.b].direction.z, 2, 32);
