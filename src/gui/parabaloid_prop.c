...
void	init_parab_prop(t_view *s)
{
	...
	s->par.pr_prop[0] = tf(s, s->space->cl_figures[s->rr.fl.y].position.x, 2, 32);
	s->par.pr_prop[1] = tf(s, s->space->cl_figures[s->rr.fl.y].position.y, 2, 32);
...
void	print_parab_prop(t_view *s)
{
	...
	{
		SDL_RenderCopy(s->rr.rend[2], s->par.pr_tex[i], NULL, &s->par.pr_rect[i]);
		SDL_RenderCopy(s->rr.rend[2], s->par.pr_prop[i], NULL, &s->par.pr_pr_rect[i]);
