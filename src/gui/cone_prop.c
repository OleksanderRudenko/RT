...
void	init_cone_prop(t_view *s)
{
	...
	s->con.cn_prop[0] = tf(s, s->space->cl_figures[s->rr.fl.y].vertex.x, 2, 32);
...
void	print_cone_prop(t_view *s)
{
	...
	{
		SDL_RenderCopy(s->rr.rend[2], s->con.cn_tex[i], NULL, &s->con.cn_rect[i]);
		SDL_RenderCopy(s->rr.rend[2], s->con.cn_prop[i], NULL, &s->con.cn_pr_rect[i]);
