...
void 	parse_cut_plane(t_iplane *fplane, t_capses *caps, JSON_Object *obj, const char *name)
{
	JSON_Object	*plane;
	...
	if (json_object_has_value_of_type(plane, "normale", JSONArray))
		...
		ft_putendl_fd("Unknown cutting plane normale. Default applied", STDERR_FILENO);
	if (json_object_has_value_of_type(plane, "point", JSONArray))
		...
		printf("point(%f %f %f)\n",fplane->point.x, fplane->point.y, fplane->point.z );
