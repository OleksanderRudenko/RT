...
/*   Updated: 2018/05/08 12:19:59 by knovytsk         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "rt.h"

static void check_radius_and_distance(JSON_Object *elipsoid, t_figure  *felipsoid)
{
  if (json_object_has_value_of_type(elipsoid, "radius", JSONNumber))
    ((t_elipsoid*)felipsoid->figure)->radius = json_object_get_number(elipsoid,
				"radius");
  else
  ...
  if (json_object_has_value_of_type(elipsoid, "rdistance", JSONNumber))
    ((t_elipsoid*)felipsoid->figure)->rdistance = json_object_get_number(elipsoid,
        "rdistance");
  else
